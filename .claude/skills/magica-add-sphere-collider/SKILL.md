---
name: magica-add-sphere-collider
description: |-
  Adds a MagicaSphereCollider to a GameObject (typically a bone).
  Magica colliders prevent cloth from penetrating character body parts.
  Common usage: add to hand, head, chest, or leg bones.
---

# Magica Cloth / Add Sphere Collider

## How to Call

```bash
unity-mcp-cli run-tool magica-add-sphere-collider --input '{
...
> ```bash
> unity-mcp-cli run-tool magica-add-sphere-collider --input-file args.json
> ```
>
> Or pipe via stdin (recommended):
> ```bash
> unity-mcp-cli run-tool magica-add-sphere-collider --input-file - <<'EOF'
> {"param": "value"}
> EOF
> ```
...
| `radius` | `number` | No | Radius of the sphere collider. Default 0.1. |
